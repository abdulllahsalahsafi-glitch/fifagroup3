...
function isLinkedLeagueGroupsCup(competition = {}) {
...
function getCompetitionExcludedMemberIds(competition = {}) {
...
function isCompetitionExcludedMember(competition = {}, memberId = "") {
  const safeId = cleanId(memberId);
  if (!safeId) return false;
  return getCompetitionExcludedMemberIds(competition).some((id) => same(id, safeId));
}

function isCompetitionExcludedMatch(match = {}, excludedIds = []) {
...
function filterCompetitionParticipantsForCalculation(competition = {}) {
...
function competitionAbsenceInfoForMember(competition = {}, memberId = "") {
  const safeId = cleanId(memberId);
...
function getApprovedCompetitionChampionName(competition = {}, fallbackName = "") {
...
function linkedCupGroupIsReady(group = {}) {
...
function knockoutBracketSizeForCount(count = 0) {
...
function normalizeCupManualPairings(pairings = []) {
...
function validateCupManualPairings(pairings = [], participants = []) {
...
function buildManualKnockoutBracketSlots(participants = [], pairings = [], size = 8) {
...
function linkedCupGroupNumber(groupKey = "", fallbackIndex = 0) {
...
function linkedCupRankText(rank = 1, groupNumber = "") {
...
function linkedCupRankSlot(group = {}, groupIndex = 0, rank = 1) {
...
function buildLinkedCupGroupPath(group = {}, groupIndex = 0, onlineMemberId = "") {
  const groupKey = group.groupKey || (groupIndex === 0 ? "A" : "B");
  const groupNumber = linkedCupGroupNumber(groupKey, groupIndex);
...
function mergeLinkedCupGeneratedMatches(existingCup = {}, generatedMatches = []) {
...
function generateLinkedLeagueGroupsCupPlan({ linkedLeague = {}, existingCup = null, onlineMemberId = "" } = {}) {
...
function buildLinkedLeagueCupDisplayCompetition(competition = {}, allCompetitions = []) {
...
function getLinkedLeagueCupParticipantRows(leagueCompetition = {}) {
...
function isSeasonCenterActiveOffer(offer = {}) {
...
function getFinanceRecordDate(row = {}) {
...
function getFinanceRecordNote(row = {}) {
